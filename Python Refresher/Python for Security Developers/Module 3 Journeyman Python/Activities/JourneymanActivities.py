...
def journeyman4():
    ...
    class Person(object):
        # Nothing comes before __init__ To give default values, do it within the __init__ function.
        def __init__(self,height = 0,weight = 0,hair_color = "black" ,eye_color = "blue"): # Default values
            self.height = height; # Class variables are by definition global.
            self.weight = weight;
            self.hair_color = hair_color;
            self.eye_color = eye_color;

        def print_info(self):
            print "Height:\t%d\nWeight:\t%d\nHair:\t%s\nEye:\t%s\n" % (self.height, self.weight, self.hair_color, self.eye_color);

    John = Person(160,50,"black","blue");
    John.print_info();
