...
public class RectangleCircleCollisionScreen extends ScreenAdapter {
    ...
    private boolean areColliding(Rectangle rectangle, Circle circle) {
        ...
        // Find closest point in rectangle from the center of the circle.
        // Then take the abs value of the distance between them. If it is < radius, then there is intersection.
        float closestX = findClosest(circle.x, rectangle.getX(),rectangle.getX() + rectangle.getWidth());
        float closestY = findClosest(circle.y, rectangle.getY(),rectangle.getY() + rectangle.getHeight());

        Vector2 closestPoint = new Vector2(closestX,closestY);

        if(Math.abs(closestPoint.dst(circle.x,circle.y)) < circle.radius){
            return true;
        }
    ...
    private float findClosest(float x, float a, float b){
        if(x < a){
            return a;
        }
        if(x > b){
