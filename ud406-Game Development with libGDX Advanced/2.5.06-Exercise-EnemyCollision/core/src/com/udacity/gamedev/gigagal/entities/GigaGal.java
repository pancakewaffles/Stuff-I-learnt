...
public class GigaGal {
    ...
    public void update(float delta, Array<Platform> platforms) {
        ...
        Rectangle gigaGalRectangle = new Rectangle(position.x - Constants.GIGAGAL_EYE_POSITION.x, position.y - Constants.GIGAGAL_EYE_POSITION.y, Constants.GIGAGAL_STANCE_WIDTH,Constants.GIGAGAL_HEIGHT);

        for (Enemy enemy : level.getEnemies()) {
            // TODO: Define enemy bounding rectangle
            // You'll want to define an enemy collision radius constant
            Rectangle enemyRectangle = new Rectangle(enemy.position.x - Constants.ENEMY_COLLISION_RADIUS,enemy.position.y-Constants.ENEMY_COLLISION_RADIUS, 2*Constants.ENEMY_COLLISION_RADIUS,2*Constants.ENEMY_COLLISION_RADIUS);
            ...
            if(gigaGalRectangle.overlaps(enemyRectangle)){
                if(gigaGalRectangle.getX() < enemyRectangle.getX() && enemyRectangle.getX() < gigaGalRectangle.getX() + gigaGalRectangle.getWidth()){
                    Gdx.app.log(TAG,"Hit from the left.");
                }
                else{
                    Gdx.app.log(TAG,"Hit from the right.");
                }
            }
