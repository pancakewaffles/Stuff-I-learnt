...
public class DifficultyScreen extends InputAdapter implements Screen {
    ...
    public void show() {
        ...
        viewport = new FitViewport(Constants.DIFFICULTYSCREEN_SIZE,Constants.DIFFICULTYSCREEN_SIZE);
    ...
    public void render(float delta) {
        ...
        renderer.circle(Constants.EASY_CENTER.x,Constants.EASY_CENTER.y,Constants.BUTTON_RADIUS);

        renderer.setColor(Constants.MEDIUM_COLOR);
        renderer.circle(Constants.MEDIUM_CENTER.x,Constants.MEDIUM_CENTER.y,Constants.BUTTON_RADIUS);

        renderer.setColor(Constants.HARD_COLOR);
        renderer.circle(Constants.HARD_CENTER.x,Constants.HARD_CENTER.y,Constants.BUTTON_RADIUS);
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if(worldTouch.y > Constants.EASY_CENTER.y - Constants.BUTTON_RADIUS && worldTouch.y < Constants.EASY_CENTER.y + Constants.BUTTON_RADIUS){
            if(worldTouch.x > Constants.EASY_CENTER.x - Constants.BUTTON_RADIUS && worldTouch.x < Constants.EASY_CENTER.x + Constants.BUTTON_RADIUS){
                game.showIciclesScreen(Constants.Difficulty.EASY);
            }
        }

        if(worldTouch.y > Constants.MEDIUM_CENTER.y - Constants.BUTTON_RADIUS && worldTouch.y < Constants.MEDIUM_CENTER.y + Constants.BUTTON_RADIUS){
            if(worldTouch.x > Constants.MEDIUM_CENTER.x - Constants.BUTTON_RADIUS && worldTouch.x < Constants.MEDIUM_CENTER.x + Constants.BUTTON_RADIUS){
                game.showIciclesScreen(Constants.Difficulty.MEDIUM);
            }
        }

        if(worldTouch.dst(Constants.HARD_CENTER) < Constants.BUTTON_RADIUS){
