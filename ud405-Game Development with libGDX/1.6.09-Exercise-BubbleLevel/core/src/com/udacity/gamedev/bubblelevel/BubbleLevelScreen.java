...
public class BubbleLevelScreen extends ScreenAdapter {
    ...
    public void render(float delta) {
        ...
        renderer.circle(WORLD_SIZE/2,WORLD_SIZE/2,WORLD_SIZE * (.5f - .25f * 9.8f / 9.8f),100);

        renderer.setColor(Color.GREEN);

        // TODO: Draw a circle to hold the bubble when the phone is flat
        renderer.circle(WORLD_SIZE/2,WORLD_SIZE/2,AXIS_WIDTH,100);

        renderer.set(ShapeType.Filled);

        // TODO: Draw the bubble
        renderer.circle(WORLD_SIZE * (.5f - .25f * yAxis / 9.8f),WORLD_SIZE * (.5f + .25f * xAxis/9.8f),AXIS_WIDTH,100);
